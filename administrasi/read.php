			...
			<div class="container">
				...
				<ul class="navbar-nav ml-auto mt-2 mt-lg-0 d-flex align-items-center">
					...
					<?PHP if ($_SESSION['role_id'] == 2) { ?>
						<li class="nav-item">
							<a class="nav-link menu active" id="kelolapasien" href="./read.php"><i class=""></i>Kelola Pendaftaran Pasien</a>
						</li>
					<?PHP } ?>
...
$stmt = $pdo->prepare('SELECT * FROM pasien ORDER BY id_pasien LIMIT :current_page, :record_per_page');
...
$num_dokters = $pdo->query('SELECT COUNT(*) FROM pasien')->fetchColumn();
?>

<div class="content read">
	<h2>Data Pasien</h2>
	<a href="./create.php" class="menu create-dokter" id="tambahDokter">Tambah Data Pasien</a>
	<table class="table-responsive">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?=$rawat['akun_id']?></td>
                <td><?=$rawat['no_rekapmedis']?></td>
                <td><?=$rawat['nama']?></td>
                <td><?=$rawat['tgl_lahir']?></td>
                <td><?=$rawat['umur']?></td>
                <td><?=$rawat['jk']?></td>
                <td><?=$rawat['pekerjaan']?></td>
                <td><?=$rawat['alamat_rumah']?></td>
                <td><?=$rawat['telepon']?></td>
                <td><?=$rawat['tanggal_masuk']?></td>
                <!-- <td><?=$rawat['dokumen']?></td> -->

                <td class="actions">
                    <a href="update.php?id_pasien=<?=$rawat['id_pasien']?>" class="menu edit" id="edit"><i class="fas fa-pen fa-xs"></i></a>
                    <a href="delete.php?id_pasien=<?=$rawat['id_pasien']?>" class="menu trash" id="hapus"><i class="fas fa-trash fa-xs"></i></a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
