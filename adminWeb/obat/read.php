...
?>
	...
	<body>
		...
		$stmt = $pdo->prepare('SELECT * FROM obat ORDER BY kd_obat LIMIT :current_page, :record_per_page');
		...
		$num_kamars = $pdo->query('SELECT COUNT(*) FROM obat')->fetchColumn();
		?>

		<div class="content read">
			<h2>Data Obat</h2>
			<a href="./create.php" class="menu create-dokter" id="tambahDokter">Tambah Data Obat</a>
			<table>
				<thead>
					<tr>
						<td>KODE</td>
				...
				<tbody>
					<?php foreach ($kamars as $obat) : ?>
						<tr>
							...
							<td class="actions">
								<a href="update.php?kd_obat=<?= $obat['kd_obat'] ?>" class="menu edit" id="edit"><i class="fas fa-pen fa-xs"></i></a>
								<a href="delete.php?kd_obat=<?= $obat['kd_obat'] ?>" class="menu trash" id="hapus"><i class="fas fa-trash fa-xs"></i></a>
							</td>
						</tr>
					<?php endforeach; ?>
				</tbody>
