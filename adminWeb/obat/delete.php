...
if (isset($_GET['kd_obat'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt = $pdo->prepare('DELETE FROM obat WHERE kd_obat = ?');
            $stmt->execute([$_GET['kd_obat']]);
            // header("Refresh:0");
...
<div class="content delete">
	<h2>Delete obat #<?=$obat['kd_obat']?></h2>
    <?php if ($msg): ?>
    <p><?=$msg?></p>
    <?php else: ?>
	<p>Anda yakin ingin menghapus obat <?=$obat['nm_obat']?>?</p>
    <div class="yesno">
    <a  href="delete.php?kd_obat=<?=$obat['kd_obat']?>&confirm=yes">Yes</a>
    <a href="delete.php?kd_obat=<?=$obat['kd_obat']?>&confirm=no">No</a>
    </div>
    <?php endif; ?>
</div>
<?php
