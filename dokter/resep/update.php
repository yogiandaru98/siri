			...
			<div class="container">
				...
				<ul class="navbar-nav ml-auto mt-2 mt-lg-0 d-flex align-items-center">
					...
					<?PHP if ($_SESSION['role_id'] == 5) { ?>
						<li class="nav-item">
							<a class="nav-link menu" id="tindakan" href="../tindakan/read.php"><i class=""></i>Tindakan</a>
						</li>
						<li class="nav-item">
							<a class="nav-link menu active" id="resepobat" href="update.php?akun_id=<?=$_SESSION['id_akun']?>"><i class=""></i>Resep Obat</a>
						</li>
					<?PHP } ?>
...
if (isset($_GET['akun_id'])) {
    if (!empty($_POST)) {
        ...
        $stmt = $pdo->prepare('UPDATE dokter SET resep_dokter = ? WHERE akun_id = ?');
        $stmt->execute([$resep_dokter, $_GET['akun_id']]);
        $msg = header("Refresh:0");
    }
 
    $stmt = $pdo->prepare('SELECT * FROM dokter WHERE akun_id = ?');
    $stmt->execute([$_GET['akun_id']]);
    $dokter = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$dokter) {
        exit('dokter doesn\'t exist with that akun_id!');
    }
} else {
    exit('No akun_id specified!');
}
?>
...
<div class="content update">
	<h2>Update Resep</h2>
    <form action="./update.php?akun_id=<?=$dokter['akun_id']?>" method="post">
