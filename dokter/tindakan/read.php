...
?>
	...
	<body>
			...
			<div class="container">
				...
				<ul class="navbar-nav ml-auto mt-2 mt-lg-0 d-flex align-items-center">
					...
					<?PHP if ($_SESSION['role_id'] == 4) { ?>
						...
						<li class="nav-item">
							<a class="nav-link menu" id="penggunaantindakan" href="./perawat/penggunaantindakan.php"><i class=""></i>Penggunaan Obat</a>
						</li>
					<?PHP } ?>
					...
					<?PHP if ($_SESSION['role_id'] == 7) { ?>
						...
						<li class="nav-item">
							<a class="nav-link menu" id="cetaktindakan" href="./pimpinan/cetaktindakan.php"><i class=""></i>Cetak Penggunaan tindakan</a>
						</li>
					<?PHP } ?>
		...
		$stmt = $pdo->prepare('SELECT * FROM tindakan ORDER BY kd_tindakan LIMIT :current_page, :record_per_page');
		...
		$num_kamars = $pdo->query('SELECT COUNT(*) FROM tindakan')->fetchColumn();
		?>

		<div class="content read">
			<h2>Data Tindakan</h2>
			<a href="./create.php" class="menu create-dokter" id="tambahDokter">Tambah Data tindakan</a>
			<table>
				<thead>
					<tr>
						<td>KODE</td>
				...
				<tbody>
					<?php foreach ($kamars as $tindakan) : ?>
						<tr>
